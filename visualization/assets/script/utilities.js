		...
		if (open == false) {
			...
			$("#mobile_menu_icon")
				.css(
					...
					"url(" +
						the_path +
						"assets/img/close-menu.svg) center center no-repeat"
				)
			...
			$("#mobile_menu_icon")
				.css(
					...
					"url(" +
						the_path +
						"assets/img/mobile-menu.svg) center center no-repeat"
				)
				.css("background-size", "55%");
		...
		if (open == false) {
			...
			$("#mobile_filter_icon")
				.css(
					...
					"url(" +
						the_path +
						"assets/img/arrow-up.svg) center center no-repeat"
				)
				.css("background-size", "55%");
			$("#select_box").show();
			...
			$("#mobile_filter_icon")
				.css(
					...
					"url(" +
						the_path +
						"assets/img/arrow-down.svg) center center no-repeat"
				)
				.css("background-size", "55%");
			$("#select_box").hide();
			open = false;
			...
			function loaded(data) {
				...
				subject_group.forEach(function (d, i) {
					console.log(
						d.key,
						d.values.length,
						precentage(d.values.length, articles)
					);
				});
				...
				console.log(
					...
					Math.round(avg_pv),
					variation_perc(avg_pv, avg_pv_prev, "avg_pv")
				);

				console.log(
					"media byte articolo",
					Math.round(avg_size),
					variation_perc(avg_size, avg_size_prev, "avg_size")
				);

				console.log(
					"media byte incipit ",
					Math.round(avg_incipit),
					variation_perc(avg_incipit, avg_incipit_prev, "avg_incipit")
				);

				console.log(
					"media byte pagina di discussione",
					Math.round(avg_discussion),
					variation_perc(
						avg_discussion,
						avg_discussion_prev,
						"avg_discussion"
				...
				console.log(
					"media avvisi",
					parseFloat(avg_issues.toFixed(3)),
					variation_perc(avg_issues, avg_issues_prev, "avg_issues")
				);

				console.log(
					"media note",
					Math.round(avg_notes),
					variation_perc(avg_notes, avg_notes_prev, "avg_notes")
				);

				console.log(
					"media immagini",
					Math.round(avg_images),
					variation_perc(avg_images, avg_images_prev, "avg_images")
				);
